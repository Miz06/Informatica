...
namespace GestioneConStato
{
    ...
    internal class Program
    {
        ...
        { //Alessandro Mizzon 4E
            ...
            const int lung = 3;
            persona[] p = new persona[lung];

            for (int i = 0; i<lung; i++)
            {
                p[i].elemento = statoElemento.Libero;
            }
            ...
            do
            {
                ...
                {
                    case (1):
                        if (p[2].elemento != statoElemento.Occupato)
                        {
                            Inserimento(ref p, ref indice, lung);
                        }
                        else
                        {
        ...
        static void Inserimento(ref persona[] p, ref int indice, int lung) //Inserimento
        {
            ...
            if (!doppio)
            {
                ...
                p[indice].elemento = statoElemento.Occupato;
                indice++;
            }
        }

        static void Visualizzazione(persona[] p, int indice, int lung) //Visualizzazione
        {
            for (int i = 0; i < lung; i++)
            {
                Console.WriteLine("------------------------");
        ...
        static int Cancella(persona[] p, int indice)  //cancella l'utente trovato e sposta tutti i successivi un posto indietro
        {
            ...
            if (t != -1)
            {
                ...
                p[t].id = "";
                p[t].elemento = statoElemento.Cancellato;
            }
            else
            {
                Console.WriteLine("persona non trovata");
