...
namespace GestioneFileLog
{
    ...
    internal class Program
    {
        ...
        { //Alessandro Mizzon 4E
            string[] opzioni = new string[] { "Inserimento", "Età", "Modifica", "Visualizzazione", "Cancellazione", "Stampa percorso file, scegli un file e stampa il contenuto",};
                ...
                switch (scelta)
                {
                    case (1):
                        if (indice < 3)
                        {
                            ...
                            ScriviFile(Path.Combine(Environment.CurrentDirectory + $"\\log{files.Length}.txt"), p[indice].ToString());
                            indice++;
                            files = Directory.GetFiles(Environment.CurrentDirectory);
                        }
                        else
                    ...
                    case (5):
                        ...
                        break;
                    case (6):
                        MenùFileDirectory(Environment.CurrentDirectory);
                        Console.WriteLine("Isnserire una scelta in base al numero");
                        int FileScelto = Convert.ToInt32(Console.ReadLine());
                        LeggiFile(files[FileScelto]);
                        break;
                }
        ...
        static void Modifica(persona[] p, int ind)
        {
            ...
            {
                Console.WriteLine("Persona non trovato");
        ...
        static void MenùFileDirectory(string d)
        {
            string[] files; 

            files = Directory.GetFiles(d); //metto nell'array i file ella directory definita

            Console.WriteLine("--------------");
            for (int i = 3; i<files.Length; i++)
            {
                Console.WriteLine($"[{i}] {files[i]}"); //menù file
            }
            Console.WriteLine("--------------");
        }
        static void ScriviFile(string d, string contenuto)
        {
        ...
        {
            StreamReader sr = File.OpenText(percorso);
            string linea;
            linea = sr.ReadLine();
